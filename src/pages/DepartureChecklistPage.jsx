...
export default function DepartureChecklistPage() {
  const [data, setData] = useState(null);
  const [newListName, setNewListName] = useState("");

  useEffect(() => {
    loadChecklist().then(setData);
  }, []);
  ...
  return (
    <div style={{ maxWidth: 1400, margin: "0 auto", padding: 20 }}>
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 18, color: C.gold, fontWeight: 700 }}>✅ Checklist départ</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 14, marginBottom: 18 }}>
        {data.lists.map((list) => (
          <ShoppingList
            ...
            onDeleteList={() => {
              if (confirm(`Supprimer la catégorie "${list.name}" ?`)) deleteList(list.id).then(setData);
            }}
          />
        ))}
      </div>
